...
public class MealyAutomaton extends Automaton {
	...
	public MealyAutomaton(int number_states, String initial_state, String[] alphabet_input, String[] alphabet_ouput) {
			...
			if (!last.isEmpty()) {
				...
				for (HashSet<String> c : sets) {
					for (int i = 0; i < alphabet_input.length && exist; i++) {
						if (!c.contains(connections.get(last).get(alphabet_input[i])[0]) && c.contains(connections.get(strings).get(alphabet_input[i])[0])) {
							exist = false;
						}
						if (c.contains(connections.get(last).get(alphabet_input[i])[0]) && !c.contains(connections.get(strings).get(alphabet_input[i])[0])) {
							exist = false;
	...
	public void createFirstPartition() {
		...
		for (Map.Entry<String, HashMap<String, String[]>> entry : connections.entrySet()) {
			String keyString = "";
	...
	public String[][] getDataToShow() {
		...
		String result[][] = new String[sets.size()][alphabet_input.length + 1];
		...
		for (int i = 0; i < result.length; i++) {
			...
			for (int j = 1; j < result[0].length; j++) {
				for (String s : sets.get(i)) {
					...
					result[i][j] = rename.get(connections.get(s).get(alphabet_input[j - 1])[0]) + " " + connections.get(s).get(alphabet_input[j - 1])[1];
					break;
				}
			}
		}
